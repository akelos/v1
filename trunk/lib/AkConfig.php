...
class AkConfig
{
    ...
    {
        if ((AK_CLI && AK_ENVIRONMENT!='testing') || (!$force && !$this->_useReadCache($environment))) return false;
        $cacheFileName = $this->_generateCacheFileName($namespace,$environment);
    ...
    {
        if ((AK_CLI && AK_ENVIRONMENT!='testing') || AK_ENVIRONMENT == 'setup' || (!$force &&!$this->_useWriteCache($environment)))  return false;
