...
define("DEFAULT_CONFIG_FILE", (dirname(__FILE__).DIRECTORY_SEPARATOR.'..'.DIRECTORY_SEPARATOR.'etc'.DIRECTORY_SEPARATOR)."nanoweb.conf");
			...
			if ($sck_connected) {
				...
				if ($htreq_headers["CONTENT-LENGTH"]) {
					...
					if (($maxblen=$conf[$vhost]["maxrequestbodylength"][0]) && ((int)$htreq_headers["CONTENT-LENGTH"]>$maxblen)) {
						...
						if ($http_continue && !$pri_err) {
						    send_response(HTTP_VERSION." ".$HTTP_HEADERS[100]."\r\n\r\n", $sck_connected);
						}

						$buf=$add_req;
						if (strlen($buf)<$htreq_headers["CONTENT-LENGTH"]) {
						    
						    $buf.=read_request($sck_connected, $dp, $pn, $htreq_headers["CONTENT-LENGTH"]-strlen($buf));
						}
