...
class mod_method_DELETE {


	var $modtype = "request_method";
	var $methods = array("DELETE");
	var $modname = "DELETE method request support";
	...
	function init() {
		...
		define("NW_METHOD_DELETE_NO_CONTENT", "204");   // 200 could be used as well
		define("NW_METHOD_DELETE_DELETED", "200");
	...
	function parser_open($uu_args, &$real_uri, &$rq_err, &$out_add_headers, &$out_contenttype) {
		...
		if (!file_exists($docroot.$real_uri)) {
			...
			$rq_err = NW_METHOD_DELETE_DELETED;
			$add_errmsg = "<b>Resource created.</b><br><br>";

		}
		else if (($res = @unlink($docroot.$real_uri))!==false){

			$rq_err = NW_METHOD_DELETE_DELETED;
			$add_errmsg = "<b>Resource deleted.</b><br><br>";

		} else {
		    $rq_err = 502;
		    $add_errmsg = "Could not delete resource. ";
