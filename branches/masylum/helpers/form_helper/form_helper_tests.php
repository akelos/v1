...
{    
    ...
    function test_for_sexy_form_helpers()
    {
        ...
        ob_start();
        $f = $form_helper->form_for('person', $person, array('url' => array('action' => 'update'), 'sexy' => true));
        $this->assertEqual(ob_get_clean(),'<form action="/url/for/test" method="post">');

        $this->assertEqual(
            $form_helper->text_field('task', 'description'),
            '<div class="field"><input id="task_description" name="task[description]" size="30" type="text" /></div>'
        );

        $this->assertEqual(
            $form_helper->text_field('task', 'description', array('object' => &$task)),
            '<div class="field"><input id="task_description" name="task[description]" size="30" type="text" value="Do the testing" /></div>'
        );

        $this->assertEqual(
            $f->text_field('person', 'name'),
            '<div class="field"><input id="person_name" name="person[name]" size="30" type="text" value="Bermi" /></div>'
        );
        ...
        $this->assertEqual(
            $f->text_field('person', 'first_name', array('size'=>80)),
            '<div class="field"><input id="person_first_name" name="person[first_name]" size="80" type="text" value="Alicia" /></div>'
        );

        $this->assertEqual(
            $form_helper->password_field('person','password'),
            '<div class="field"><input id="person_password" name="person[password]" size="30" type="password" /></div>'
        );

        $this->assertEqual(
            $form_helper->file_field('person','photo'),
            '<div class="field"><input id="person_photo" name="person[photo]" size="30" type="file" /></div>'
        );

        $this->assertEqual(
            $form_helper->hidden_field('person','referer'),
            '<div class="field"><input id="person_referer" name="person[referer]" type="hidden" /></div>'
        );

        $this->assertEqual(
            $form_helper->text_area('person','notes'),
            '<div class="field"><textarea cols="40" id="person_notes" name="person[notes]" rows="20"></textarea></div>'
        );

        $this->assertEqual(
            $form_helper->text_field('person','name'),
            '<div class="field"><input id="person_name" name="person[name]" size="30" type="text" /></div>'
        );
        ...
        $this->assertEqual(
            $f->password_field('person','password'),
            '<div class="field"><input id="person_password" name="person[password]" size="30" type="password" value="1234" /></div>'
        );
        ...
        $this->assertEqual(
            $f->hidden_field('person','referer'),
            '<div class="field"><input id="person_referer" name="person[referer]" type="hidden" value="http://www.example.com" /></div>'
        );

        $this->assertEqual(
            $f->text_area('person','notes'),
            '<div class="field"><textarea cols="40" id="person_notes" name="person[notes]" rows="20">Check this &quot;NOTES&quot;</textarea></div>'
        );
        ...
        $this->assertEqual(
            $f->check_box("post", "validate"),
            '<div class="field"><input name="post[validate]" type="hidden" value="0" />'.
            '<input checked="checked" class="check_box" id="post_validate" name="post[validate]" type="checkbox" value="1" /></div>'
        );

        $this->assertEqual(
            $f->radio_button('post', 'validate','si'),
            '<div class="field"><input class="radio_button" id="post_validate_si" name="post[validate]" type="radio" value="si" /></div>'
        );

        $this->assertEqual(
            $f->radio_button('post', 'validate','1'),
            '<div class="field"><input checked="checked" class="radio_button" id="post_validate_1" name="post[validate]" type="radio" value="1" /></div>'
        );
    }
}
