...
class FormHelper extends AkActionViewHelper
{
    ...
    function form_for($object_name, &$object, $options = array())
    {
        ...
        $this->sexy_form = isset($options['sexy']) ? $options['sexy'] : false;
        unset($options['sexy']);
    ...
    {
        return $this->_field($object_name, $column_name, $tag_options, array_merge($options, array('type' => 'text')));
    }
    ...
    {
        return $this->_field($object_name, $column_name, $tag_options, array_merge($options, array('type' => 'password')));
    }
    ...
    {
        return $this->_field($object_name, $column_name, $tag_options, array_merge($options, array('type' => 'hidden')));
    }
    ...
    {
        return $this->_field($object_name, $column_name, $tag_options, array_merge($options, array('type' => 'file')));
    }
    ...
    {
        return $this->_field($object_name, $column_name, $tag_options, array_merge($options, array('type' => 'text_area')));
    }
    ...
    {
        $options = array_merge(array('tag_value' => $tag_value, 'type' => 'radio_button'), $options);
        return $this->_field($object_name, $column_name, $tag_options, $options);
    }
    ...
    {
        $options = array_merge(array('checked_value' => '1', 'unchecked_value' => '0', 'type' => 'check_box'), $options);
        return $this->_field($object_name, $column_name, $tag_options, $options);
    }
    ...
    /**
      ...
      */
    function submit_button($value, $tag_options = array())
    {
        $button = TagHelper::tag('input', array_merge(array('type' => 'submit', 'value' => $value), $tag_options));
        return TagHelper::content_tag('div', $button, array('class' => 'submit_field'));
    }
	
    /**
      * Returns a question mark link.
      */
    function question_mark_link()
    {
        return TagHelper::content_tag('div', "<a href='#' title='Need more help about this field?'>[?]</a>", array('class' => 'question_mark'));
    }
    ...
    {
        return TagHelper::content_tag('small', $help_content, array('class' => 'help_box hidden'));
    }
    ...
    function error_box($model, $column_name)
    {
		if($model && $model->hasErrors()){
	        ...
	        return TagHelper::content_tag('div', $errors_content, array('class' => 'error_box hidden'));
		}else {
			return '';
		}
    }
    ...
    * @access private
    */
    private function _field($object_name, $column_name = null, $tag_options = array(), $options = array())
    {   
        // If we provide only one attribute
        if(!$column_name && isset($this->object_name)){
            $column_name = $object_name;
            $object_name = $this->object_name;
		// If we skip object_name 
        }elseif(is_array($column_name) && !empty($column_name) && isset($this->object_name)){
			$options     = $tag_options;
		...
		switch ($options['type']) {
            ...
            case 'password':
            case 'text':            
                $field = $InstanceTag->to_input_field_tag($options['type'], $tag_options);
                break;
            case 'text_area':
            ...
            case 'radio_button':
                if($this->sexy_form){
                    $tag_options['class'] = empty($tag_options['class']) ? 'radio_button' : 'radio_button '.$tag_options['class'];
                }
                $field =  $InstanceTag->to_radio_button_tag($options['tag_value'], $tag_options);
                break;
            case 'check_box':
                if($this->sexy_form){
                    $tag_options['class'] = empty($tag_options['class']) ? 'check_box' : 'check_box '.$tag_options['class'];
                }
		...
		return  ($this->sexy_form) ?  render_sexy_form($options, $field, $object, $column_name) : $field;
    }
     ...
     */
	private function render_sexy_form($options, $field, $object, $column_name){
		$label         = empty($options['label']) ? '' : $InstanceTag->to_label_field_tag($options);
		$question_mark = empty($options['help_box']) ? '' : $this->question_mark_link();
				
		$help_box  = empty($options['help_box']) ? '' : $this->help_box($options['help_box']);
		$error_box = $this->error_box($object, $column_name);
		
		return TagHelper::content_tag('div', $label.$question_mark.$help_box.$field, array('class' => 'field')).$error_box;
	}
}
...
class AkFormHelperBuilder extends FormHelper
{
    function AkFormHelperBuilder($object_name, &$object, &$template)
    {
        ...
        $this->sexy_form = $template->sexy_form;
    }
}
