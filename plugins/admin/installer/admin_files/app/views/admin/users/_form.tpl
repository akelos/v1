...
<fieldset>
    ...
    {loop Roles}
        ...
        <li>
            ...
            <input tabindex="3" type="checkbox" id="roles_id-{Role.id}" name="roles[{Role.id}]" {?is_checked}checked="checked"{end} />
...
<fieldset>
    <p>
        ...
        <input id="user_password" name="user[password]" size="30" tabindex="6" type="password" /> {?User.id}<span class="information">_{leave empty in order to keep previous password}</span>{end}
    </p>
    <p>
        <label {!User.id}class="required"{end} for="user_password_confirmation">_{Password confirmation}</label>
        <input id="user_password_confirmation" name="user[password_confirmation]" size="30" tabindex="7" type="password" />
    </p>
</fieldset>
