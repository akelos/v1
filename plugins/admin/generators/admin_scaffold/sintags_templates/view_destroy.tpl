<div id="content_menu">
    <ul class="menu">
        ...
        <li class="active"><?php  echo '<%='?> link_to _('Deleting <?php  echo AkInflector::humanize($singular_name)?>'), :action => 'destroy', :id => <?php  echo $model_name?>.id %></li>
...
<?php  echo '<%='?>  start_form_tag :action => 'destroy', :id => <?php  echo $model_name?>.id %>
    ...
    <dl>
        ...
        {loop content_columns}
          ...
          <dd><?php  echo '<?php  echo '?> $<?php  echo $model_name?>->get($content_column) ?>&nbsp;</dd>
        {end}
    </dl>
